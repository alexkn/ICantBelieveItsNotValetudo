...
/**
    ...
    * @constructor
    */
function DRAW_MAP_PNG(options, callback) {
    const settings = Object.assign({
        ...
        colorBackground: "#33a1f5",
        colorBackground2: "#046cd4",
        colorFloor: "#56affc",
        colorObstacleWeak: "#b3edff",
        colorObstacleStrong: "#a1dbff",
        colorPath: "#ffffffff",
        colorForbiddenMarker: "#ff0000ff",
        colorForbiddenZone: "#ff000060"
    }, options.settings);

    const COLORS = {
        background: Jimp.cssColorToHex(settings.colorBackground),
        background2: Jimp.cssColorToHex(settings.colorBackground2),
        floor: Jimp.cssColorToHex(settings.colorFloor),
        obstacle_weak: Jimp.cssColorToHex(settings.colorObstacleWeak),
        obstacle_strong: Jimp.cssColorToHex(settings.colorObstacleStrong),
        path: Jimp.cssColorToHex(settings.colorPath),
        forbidden_marker: Jimp.cssColorToHex(settings.colorForbiddenMarker),
        forbidden_zone: Jimp.cssColorToHex(settings.colorForbiddenZone)
    };   
    ...
    const drawGradientBackground = function(image) {
    ...
    const drawMapForViewport = function(image) {
    ...
    const drawForbiddenZones = function (image) {
    ...
    const drawVirtualWalls = function(image) {
    ...
    const drawPath = function(image) {
        if (settings.drawPath === true) {
    ...
    const drawCharger = function(image, chargerImage) {
    ...
    const drawRobot = function(image, robotImage) {
    ...
    const drawImage = async function() {
        try {
            // start async loading
            let loadImage = Jimp.create(BOUNDS.x2-BOUNDS.x1, BOUNDS.y2-BOUNDS.y1, COLORS["background"]);
            let loadChargerImage = Jimp.read(chargerImagePath);
            let loadRobotImage = Jimp.read(robotImagePath);

            // draw elements
            let image = await loadImage;
            drawGradientBackground(image);
            drawMapForViewport(image);
            drawForbiddenZones(image);
            drawVirtualWalls(image);
            image.scale(settings.scale, Jimp.RESIZE_NEAREST_NEIGHBOR);
            drawPath(image);
            drawCharger(image, await loadChargerImage);
            drawRobot(image, await loadRobotImage);
